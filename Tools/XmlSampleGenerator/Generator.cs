namespace Microsoft.Xml.XMLGen
{
    ...
    internal abstract class XmlValueGenerator
    {
        ...
        internal static XmlValueGenerator TryCreateGenerator(XmlSchemaDatatype datatype, int listLength)
        {
            try
            {
                return CreateGenerator(datatype, listLength);
            }
            catch
            {
                var generator = new Generator_unsupportedType();
        ...
        private static XmlValueGenerator CreateUnionGenerator(XmlSchemaDatatype dtype, CompiledFacets facets, int listLength)
        {
            ...
            {
                union_genr.AddGenerator(XmlValueGenerator.TryCreateGenerator(st1.Datatype, listLength));
            }
            return union_genr;
        }
        ...
        private static XmlValueGenerator CreateListGenerator(XmlSchemaDatatype dtype, CompiledFacets facets, int listLength)
        {
            ...
            list_genr.AddGenerator(XmlValueGenerator.TryCreateGenerator(itemType, listLength));
            return list_genr;
        }
    ...
    {
        private readonly string value = "anyType";
    ...
    {
        private readonly string value = "anySimpleType";
    ...
    internal class Generator_unsupportedType : Generator_facetBase
    {
        public override string GenerateValue()
        {
    ...
    internal class Generator_float : XmlValueGenerator
    {
        ...
        private readonly float step = 1.1F;
        private float maxBound = float.MaxValue;
        private float minBound = float.MinValue;
    ...
    {
        private TimeSpan startValue = XmlConvert.ToTimeSpan("P1Y1M1DT1H1M1S");
        private readonly TimeSpan step = XmlConvert.ToTimeSpan("P1Y");
        private TimeSpan increment = new TimeSpan(0, 0, 0, 0);
        private readonly TimeSpan endValue = new TimeSpan(1, 0, 0, 0);
        private TimeSpan minBound = new TimeSpan(TimeSpan.MinValue.Days, TimeSpan.MinValue.Hours, TimeSpan.MinValue.Minutes, TimeSpan.MinValue.Seconds, TimeSpan.MinValue.Milliseconds);
        private TimeSpan maxBound = new TimeSpan(TimeSpan.MaxValue.Days, TimeSpan.MaxValue.Hours, TimeSpan.MaxValue.Minutes, TimeSpan.MaxValue.Seconds, TimeSpan.MaxValue.Milliseconds);
    ...
    {
        private Generator_integer binGen = new Generator_int();
    ...
    { //A derived type of token
        private static string[] languageList = new string[] { "en", "fr", "de", "da", "el", "it", "en-US" };
    ...
    {
        private ArrayList unionGens = new ArrayList();
    ...
    {
        private XmlValueGenerator genr;
